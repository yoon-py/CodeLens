...
> `lensme serve`/`mcp`(4툴)/`impact-check`/`hotspots`/`diff` E2E 통과.
...
## Phase 1 — 스키마 v1 확정 + 파이프라인 보강 (lensme/ Python)
...
## Phase 2 — UI 파운데이션 (lensme/ui/ React)
...
| # | 작업 | 내용 |
|---|------|------|
| D1 | `lensme serve` | UI 빌드 산출물을 Python 패키지에 번들, ontology.json과 함께 로컬 서빙 + 브라우저 오픈. **time-to-wow: `graphify . && lensme build && lensme serve` 3줄** |
| D2 | graphify 연동 | graphify post-commit hook / --watch 뒤에 lensme build 자동 실행 (그래프 갱신→온톨로지 갱신). CodeSee가 죽은 이유(지도-코드 drift)를 구조적으로 회피 |
...
| # | 작업 | 내용 |
|---|------|------|
| R1 | 독립 repo 분리 + 이름 확정 | PyPI/GitHub 이름 충돌 확인 후 결정 (lensme은 작업명) |
