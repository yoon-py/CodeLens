...
def impact_check_report(onto: dict, changed_paths: list[str]) -> str:
    ...
    if not lines:
        return ""
    header = "lensme impact-check (informational, never blocks):"
    return "\n".join([header, *lines])
...
HOOK_BODY = """#!/bin/sh
# installed by `lensme impact-check --install-hook` - informational only
lensme impact-check --ontology {ontology} --repo "$(git rev-parse --show-toplevel)" || true
exit 0
"""
...
def install_hook(repo: str, ontology: str) -> str:
    ...
    if target.exists() and "lensme impact-check" not in target.read_text(encoding="utf-8"):
        raise SystemExit(
            f"{target} already exists and is not ours - append this line yourself:\n"
            f"  lensme impact-check --ontology {Path(ontology).resolve()}"
        )
